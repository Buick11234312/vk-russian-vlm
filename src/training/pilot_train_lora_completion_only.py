...
SAMPLE_PATH = Path("data/splits/gqa_pilot_sample_completion_only.json")
OUTPUT_DIR = Path("outputs/pilot_lora_completion_only")
...
def sample_rows(
    ...
    eligible = [row for row in rows if row["imageId"] in allowed_image_ids]
    if len(eligible) < n:
        raise RuntimeError(f"Need {n} eligible rows, found only {len(eligible)}.")
    rng = random.Random(seed)
    indices = rng.sample(range(len(eligible)), k=n)
    return [eligible[index] for index in indices]


def load_selected_rows() -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    if not SPLIT_PATH.exists():
        raise FileNotFoundError(f"Split file not found: {SPLIT_PATH}")

    split_data = json.loads(SPLIT_PATH.read_text(encoding="utf-8"))

    train_image_ids = set(split_data["train_image_ids"])
    validation_image_ids = set(split_data["validation_image_ids"])

    overlap = train_image_ids & validation_image_ids
    if overlap:
        raise RuntimeError(f"Image leakage in split file: {len(overlap)} overlaps.")
    ...
    train_selected_images = {row["imageId"] for row in train_rows}
    validation_selected_images = {row["imageId"] for row in validation_rows}
    selected_overlap = train_selected_images & validation_selected_images

    if selected_overlap:
        raise RuntimeError("Pilot train/validation image leakage detected.")
        ...
        json.dumps(
            {
                ...
                "train_question_ids": [row["id"] for row in train_rows],
                "validation_question_ids": [row["id"] for row in validation_rows],
                "train_unique_images": len(train_selected_images),
                "validation_unique_images": len(validation_selected_images),
...
def load_images(selected_rows: list[dict[str, Any]]) -> dict[str, Any]:
    target_image_ids = {row["imageId"] for row in selected_rows}
    ...
    if missing:
        raise RuntimeError(f"Missing image IDs: {sorted(missing)[:20]}")
...
def main() -> None:
    print("=" * 80)
    print("GQA PILOT LORA TRAINING - COMPLETION ONLY LOSS")
    print("=" * 80)

    train_rows, validation_rows = load_selected_rows()
    image_by_id = load_images(train_rows + validation_rows)

    train_dataset = to_vlm_dataset(train_rows, image_by_id)
    validation_dataset = to_vlm_dataset(validation_rows, image_by_id)
    ...
    visual_trainable = [
        name for name in trainable_names if "visual" in name
    ]
    ...
    metrics = {
        ...
        "eval_loss_change": post_eval_loss - pre_eval_loss,
    ...
    print()
    print("=" * 80)
    print("PILOT TRAINING COMPLETE - COMPLETION ONLY")
    print("=" * 80)
    ...
    print(f"Eval loss change:     {post_eval_loss - pre_eval_loss:+.6f}")
